// Simplified server.js for Render.com free tier
...
// Simple in-memory data storage for free tier (no MongoDB dependency)
const data = {
  jobs: [],
  cvs: [],
  applications: [],
...
try {
  ...
  if (fs.existsSync(cvsDir)) {
    ...
    data.cvs = cvFiles.map(file => ({
      id: file,
      filename: file,
      path: path.join(cvsDir, file),
      uploadDate: new Date(),
      type: file.includes('Financial') ? 'Financial Analyst' : 
            file.includes('Business') ? 'Business Analyst' : 
            file.includes('Budget') ? 'Budget Analyst' : 
            file.includes('ERP') ? 'ERP Business Analyst' : 'Other',
      experienceLevel: file.includes('_I.') ? 'Entry Level' :
                      file.includes('_II.') ? 'Mid Level' :
                      file.includes('_III.') ? 'Senior Level' :
                      file.includes('_IV.') ? 'Expert Level' : 'Unknown'
    }));
  }
